...
const Mori = {
    ...
    evalNode(node, env) {
        ...
        switch (node.type) {
            ...
            case 'Evaluatable':
                ...
                let notRegex = /(not)/;
                let notMatch = notRegex.exec(node.op.value);
                ...
                let negation = notMatch ? true : false;
                if(node.op.negation){
                    //When we do a comparison like
                    //there are more than 7 farmers
                    //We end up with 7 > farmers and the operator has a negation bool.
                    //So here we swap it to make it what we meant correctly.
                    negation = !negation;
                }
                ...
                if (greaterThanMatch && orEqualMatch) {
                    ...
                    if (node.op.value.startsWith('are') && node.op.value != 'are') {
                        node.op.value = node.op.value.substring(4);
                    }
                    
                    if (node.op.value.startsWith('not') && node.op.value != 'not') {
                        node.op.value = node.op.value.substring(4);
                        negation = true;
                    }
            ...
            case 'Action':
                ...
                if (node.action.value == 'hire' || node.action.value == 'fire' || node.action.value == 'relocate' || node.action.value == 'assign' || node.action.value == 'unassign') {
                    if (!node.target) {
                    ...
                    if (node.action.value == 'hire') {
                        ...
                        actual = this.vueApp.fire(prof, amount);
                        this.vueApp.hire(relocTarget, actual);
                    }
                    else if (node.action.value == 'fire') {
                        actual = this.vueApp.fire(prof, amount);
                    }
                    else if (node.action.value == 'assign') {
                        actual = this.vueApp.tryAssign(prof, amount);
                    }
                    else if (node.action.value == 'unassign') {
                        actual = this.vueApp.tryUnassign(prof, amount);
                    }

                    if (actual == amount) {
                        if(node.action.value == 'assign' || node.action.value == 'unassign'){
                            this.vueApp.consoleOutputs.push(`${node.action.value}ed ${amount} ${outputProfName} to assigned children.`);
                        }
                        else if (node.action.value == 'relocate'){
                            let relocTarget = this.vueApp.sanitizeProfName(node.relocateTarget.value);
                            this.vueApp.consoleOutputs.push(`${node.action.value}d ${amount} ${outputProfName} to ${relocTarget.Name}.`);
                        }
                        else{
    ...
    resolveAntecedents(node, context = {}) {
        if(!node){
            console.error("Tried to resolve antecedents of a null node, context was", context);
            return null;
        }
        if (node.type == "Evaluatable") {
            if (node.rhs && node.rhs.type == "MissingAntecedent") {
                node.rhs = context.defaultSubject;
            }

            return node;
        }

        if (node.type == "Action" && node.untilClause) {
            //In the case of assign and unassign, we need to assume that the antecedent of the until clause is the children, not the target.
            if(node.action.value == 'assign' || node.action.value == 'unassign'){
                node.untilClause = this.resolveAntecedents(node.untilClause, {
                    defaultSubject: {
                        type:"IDENT",
                        value: "assigned children",
    ...
    parse(tokens) {
        ...
        for(let command of ast){
            command = this.resolveAntecedents(command);
        }
    ...
    parseEvaluatable() {
        ...
        if (next.type == 'THERE_ARE') {
            ...
            if (optionalComparator.type == "COMPARATOR") {
                //We need to handle the case there are more than 7 farmers.
                //Right now this ends up as 7 > farmers, which is the opposite.
                //Options are to swap in place or to
                optionalComparator = this.next();
                operator = optionalComparator;
                operator.negation = true;
            }
            else if (optionalComparator.type == "MORE") {
                //There are more farmers than x.
                this.consume();
                operator = {
                    type: "COMPARATOR",
                    value: ">",
            ...
            if (next.type == "NUMBER") {
                ...
                else if (next.type == "IDENT" || next.type == "NUMBER") {
                    rhs = this.parseExpression();
                }
                else {
                    //Something like "there are 10."" will reach here. We mark it as missing and then come back later to fill in whatever the antecedent probably was.
                    rhs = {
                        type: "MissingAntecedent",
                        line: next.line,
                        id: next.id
                    };
                }
    ...
    tokenize(input) {
        const tokenSpec = [
            ...
            ['SKIP', /^[ \t\n]+/],
            ['SKIP', /^(a )\b/],
            ["SKIP", /^(children to|child to)\b/], //This allows for assign and assign children to to have the same syntax without changing the code at all. Woo! 
            ['NUMBER', /^\d+/],
            ...
            ['COMPARATOR', /^((?:is not|are not)* *(?:greater|more|less|fewer|equal) *(?:than)* *(?:or equal )* *(?:to|with)*)/i],
            ['COMPARATOR', /^((?:is|are)* *(?:greater|more|less|fewer|equal) *(?:than)* *(?:or equal )* *(?:to|with)*)/i],
            ['COMPARATOR', /^(is not the same as|are not the same as)/i],
            ['COMPARATOR', /^(is the same as|are the same as)/i],
            ['COMPARATOR', /^(is not >|is not <|is not <=|is not >=|is not =|is not ==|are not >|are not <|are not <=|are not >=|are not =|are not ==|not >=|not <=|not >|not <|not ==)/],
            ...
            ['IT_IS', /^it is\b/],
            ['ACTION', /^(hire|fire|build|demolish|relocate|assign|unassign)\b/],
    ...
    textNumberToNumber(text) {
        ...
        if (text == "no" || text == "zero" || text == "none") {
            return "0";
        }
