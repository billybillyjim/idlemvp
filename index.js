        ...
        methods:{
            ...
            getTechnologies(){
                ...
                return this.technologies.filter(x => {
                    ...
                    let hasAllTech = false;
                    const requiredTechs = x.Requirements?.Technologies || [];

                    // If no requirements, tech is visible by default
                    if (requiredTechs.length === 0) {
                        hasAllTech = true;
                    }
                    else{
                        hasAllTech = requiredTechs.every(reqName => {
                            const requiredTech = allTechByName[reqName];
                            if (!requiredTech) {
                                return false;
                            }
                            return requiredTech.isLocked === false;
                        });
                    }
                    return x.Visible && hasAllTech});
            },
            ...
            processUnmetDemand(){
                for(let currency of Object.values(this.currencydata)){
                    ...
                    if(change >= 0 && this.demand[currency.Name]){
                        ...
                        if(this.currencyDemandDescriptions[currency.Name]){
                            this.currencyDemandDescriptions[currency.Name]['Produced'] = change + '\n';
                        }
            ...
            getDailyDemandDescription(currency){
                ...
                for(let prof of Object.keys(this.currencyDemandDescriptions[currency.Name] ?? {})){
                    output += prof + ': ' + this.currencyDemandDescriptions[currency.Name][prof] + '\n'; 
                }

                return output;
            },
            processBaseGrowth(){
                this.currencydata.Food.Amount += 5;
                this.currencyProductionDescriptions['Food'] = {};
                this.currencyProductionDescriptions['Food']['Base'] = 5;
                this.currencydata.Knowledge.Amount += this.getPopulation() * 0.002;
                this.currencyProductionDescriptions['Knowledge'] = {};
                this.currencyProductionDescriptions['Knowledge']['From Population'] = this.getPopulation() * 0.002;
            ...
            processCosts(){
                this.currencyDemandDescriptions['Food'] = {};
                for(let profession of this.professions){
                    this.currencydata.Food.Amount -= profession.Count;
                    this.currencyDemandDescriptions['Food'][profession.Name] = profession.Count * 1;
                    if(!this.buy(profession.Cost, profession.Count)){
                        for (const key in profession.Cost) {
                            if (Object.hasOwn(this.unmetdemand, key)) {
                                this.unmetdemand[key] += profession.Cost[key];
                                this.currencyDemandDescriptions[key][profession.Name] += profession.Cost[key];
                            } else {
                                this.unmetdemand[key] = profession.Cost[key];
                                this.currencyDemandDescriptions[key] = {};
                                this.currencyDemandDescriptions[key][profession.Name] = profession.Cost[key];
                            }
                        }
                    }
            ...
            getPrecalendarDate(date){
                ...
                if(timeOfDay < currentSunrise || timeOfDay > currentSunset){
                    return 'The night is ' + nightLength + ' and the weather is ' + weatherDescription;
                }
                return 'The day is ' + sunLength + ' and the weather is ' + weatherDescription;
            },
